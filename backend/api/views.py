...
def download_dress_rental_pdf(request, pk):
    ...
    return generate_pdf(response, "Dress Rental Contract", client, rental.id, rental.start_date, items, rental.total_price, f"من {rental.start_date} إلى {rental.end_date}")
...
def download_dress_rental_pdf(request, pk):
    ...
    # Create a dummy item object for the common generator
    class RentalItem:
        def __init__(self, name, price):
            self.item = type('obj', (object,), {'name': name})
            self.quantity = 1
...
def generate_pdf(response, title_text, client, ref_id, date_str, items, total_amount, footer_note=""):
    def ar(text):
        if not text: return ""
        try:
            reshaped_text = arabic_reshaper.reshape(str(text))
    ...
    # 1. Header
    elements.append(Paragraph(ar("Rosa Event"), style_title))
    contact_data = [["Facebook: Roza event", "Instagram: roza.rose8."], ["Tiktok: roza event kalaa", "Whatsapp: 0662605544"]]
    contact_table = Table(contact_data, colWidths=[200, 200])
    contact_table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER'), ('TEXTCOLOR', (0,0), (-1,-1), colors.HexColor('#4B5563')), ('FONTSIZE', (0,0), (-1,-1), 9)]))
    elements.append(contact_table)
    ...
    # 2. Info Box
    box_data = [
        [Paragraph(ar(f"<b>التاريخ :</b> {date_str}"), style_normal), Paragraph(ar(f"<b>الزبون:</b> {client.first_name} {client.last_name}"), style_normal)],
        [Paragraph(ar(f"<b>رقم المرجع:</b> {ref_id}"), style_normal), Paragraph(ar(f"<b>الهاتف:</b> {client.phone}"), style_normal)]
    ]
    info_table = Table(box_data, colWidths=[240, 240])
    info_table.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor('#F9FAFB')), ('ALIGN', (0,0), (-1,-1), 'RIGHT'), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#E5E7EB')), ('PADDING', (0,0), (-1,-1), 12)]))
    ...
    # Handle different item structures (Ngafa uses method, Salon uses direct fields in serializer but here we have ORM objects)
    # SalonVisit.services is a ManyToManyField of SalonService (name, price)
    # DressRental has just one dress, wrapped above.
    
    for item in items:
        # Determine fields based on object type or duck typing
        name = getattr(item.item if hasattr(item, 'item') else item, 'name', 'Unknown')
        
        # Quantity
        qty = getattr(item, 'quantity', 1) # SalonService doesn't have qty on the relationship usually, so default 1
        
        # Price
        # For Salon, price is on the service object. For Ngafa/Rental, it might be on the link or passed in.
        # In SalonVisit wrapper below, I am passing `visit.services.all()`. Service object has `price`.
        if hasattr(item, 'price_at_booking'):
            price = item.price_at_booking
        elif hasattr(item, 'price'): # SalonService has price
            price = item.price
        else:
            price = 0
